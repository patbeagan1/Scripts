          ...
          case $key in
            ...
            hwidmatch)
              ...
              if [ -n "$hwidprefix" ]; then
                ...
                # If prefix is a single word, append a space to
                # the end of it since all regexps are like:
                #   "^PEPPY .*"
                tmphwidprefix=$hwidprefix
                if [[ $hwidprefix =~ $singlewordre ]]; then
                  tmphwidprefix=$hwidprefix$space
                fi
  
                # Eval prefix against regexp.
                if [[ $tmphwidprefix =~ $curregexp ]]; then
                  showregexp=true
                fi
              fi
              ;;
            hwid)
              ...
              if [ -z "$hwidprefix" ] ||
                 [[ $curhwid == "$hwidprefix"* ]] ||
                 [[ $curhwid == "SAMS $hwidprefix"* ]] ||
                 [[ $curhwid == "ACER $hwidprefix"* ]] ||
                 [[ $curhwid == "IEC $hwidprefix"* ]]; then
                # No prefix, or HWID starts with prefix.
                # Special case for SAMS/ACER/IEC where the device name
                # is the second word in the HWID not the first.
                if [ -n "$curregexp" ] && [[ $curhwid == "$hwidprefix"* ]]; then
                  showregexp=true
                else
                  showhwid=true
                fi
